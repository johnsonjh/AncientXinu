...
/* IIASA fix (from MIT) catches an error that wasn't caught before */
		if (o==NAME && csym->hclass==TYPEDEF) {	/* these three lines*/
			if (tkw >= 0)			/* are the whole */
				error("type clash");	/* fix */
			tkw = csym->htype;
