...
loop:
	...
	case LCON:
		printf("$%o", flag>10?
#ifdef vax
		p->lvalue.intx[0]:p->lvalue.intx[1]);
#else
		p->lvalue.intx[1]:p->lvalue.intx[0]);
#endif
		return;
	...
	case AUTOI:
		printf("(r%d)%s", p->nloc, flag==1?"":"+");
		return;

	case AUTOD:
		printf("%s(r%d)", flag==2?"":"-", p->nloc);
		return;
...
#ifdef vax
		op = geti();		/* Longs in other order on vax */
		t = geti();
#else
		t = geti();
		op = geti();
