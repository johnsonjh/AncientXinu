/*----------------------------------------------------------------------*/
/*									*/
/*	download	+++ load programs into LSI 11/02 +++		*/
/*									*/
...
char *argv[];
{
	 ...
	 */
	if (getlsi(lsinum, &lsinum, &Lsifd, lockfile) < 0)
		exit(1);
	Openfiles[Lsifd] = A.lsiline = lsidevs[lsinum];
	if ( !A.silent )
		printf("Using LSI #%d\n", lsinum);
	aoutfd = openfile(A.filename,0);
	ldrfd = openfile(A.loader,0);
	linemode(Lsifd,LSISPEED,RAW);
	/*
	 * download sequence -
	 *    get ODT's attention
	...
	setreg(Lsifd,1,ahdr.a_bss);
	setreg(Lsifd,2,(A.autostart ? A.startdelay : -1));
	setreg(Lsifd,3,ahdr.a_entry & (~1));
	setreg(Lsifd,4,LSIPORT);
        message("\rstarting boot loader...");
	sendodt(Lsifd,"P",FALSE);
        fastload(aoutfd, &ahdr);
	autostart(Lsifd,&ahdr);
        message("done");
	touch(lockfile);
	callexit(0);
}
...
char *argv[];
{
	...
	for ( arg=1 ; arg<argc ; arg++ ) {
		if ( argv[arg][0] == '-' ) {
			...
			case 'l':
				argv[arg] += 2;
				if ( (lsinum=atoi(argv[arg])) < 0)
					usagexit(argv[0]);
				A.lsiline = lsidevs[lsinum];
				break;
			case 'b':
				if ( ++arg >= argc )
					usagexit(argv[0]);
...
struct exec *hdr;
{
	...
	setreg(Lsifd,0,MAXADDR);
	setreg(Lsifd,1,stklen);
        setreg(Lsifd,7,LOADSTART);
        setreg(Lsifd,-1,INTDISABLE);
        sendodt(Lsifd,"P", FALSE);
        sleep(1);
	for ( i=stklen-1 ; i>=0 ; i-- ) {
		write(Lsifd, &stack[i], 1);
		if ( A.verbose )
	...
	while ( words > 0 ) {
		...
		do {
			sendpack(addr,length,buf);
                        ...
                        if ( read ( Lsifd, &resp, 1) != 1 ) {
				perror(Openfiles[Lsifd]);
				exit(1);
                        }
                        canceltimer();
                        if ( A.verbose )
