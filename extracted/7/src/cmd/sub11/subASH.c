/*
 *      ARITHMETIC SHIFT CONVERSION ROUTINE
 *
 * TO COMPILE RUN cc -O subASH.c
 *
 *   Given an LSI-11 arithmetic shift instruction
 *      ash A,R
 * this program will generate assembly code that will produce the equivalent
 * result. Condition codes are properly set.
 *   It replaces each "l:ash A,r0" line (for example) with:
 *      mfps    -(sp)                   / get psw
...
#define ASH "ash"       /* instruction to be replaced */
...
char **argv;
{
        ...
        while (fgets(line, BUFSIZ, stdin) != NULL){
                ...
                if (findash(line)){
                        ...
                        printf("\tmov\t%s,-(sp)\n", foperand);
                        printf("\tmov\t%s,-(sp)\n", soperand);
                        printf("\tjsr\tpc,shift2\n");
                        printf("\tmov\t(sp)+,%s\n", soperand);
                        printf("\ttst\t(sp)+\n");
                        printf("\tmtps\t(sp)+\n");
                }
                else    /* not an "ash" instruction - print whatever's there */
                        printf("%s", line);
        }
...
char *s;
{
        ...
        /* if not ASH, don't go on */
        if (strcmp(operator, ASH) != 0)
                return(0);
        ...
        if (*foperand == '\0' || *soperand == '\0'){
                fprintf(stderr, "%s, line %d: ash takes 2 operands\n", pgnm, linect);
                return(0);
        }
