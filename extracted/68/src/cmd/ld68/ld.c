...
int nuxiflag;	/* reorder output for 68000 */
 ...
 */
long	textbase, database, bssbase;
...
char **argv;
{
	...
	for (c=1; c<argc; c++) {
		...
		case 'B':
			if (++c >= argc)
				error(1, "-B: arg missing");
			if (tsize!=0)
				error(1, "-B: too late, some bss already loaded");
			bssbase = htoi(*p++);
			continue;
		case 'l':
			save = ap[--i]; 
	...
	struct biobuf *b1, *b2;
{
	...
	struct local *lp;
	long tw;
	struct relocation_info *rp, *rpend;
	...
	for (rp = relp; rp < rpend; rp++) {
		...
		if (!nuxiflag) 
			...
			Xcp = cp;
			tw = 0;
			for(i=getsize(rp->r_length);i>=0;i--) {
				unsigned char byte;
