...
struct	cmdname	{			/* list of valid commands	*/
	...
	int	cargs;
	char	*cexpl;
} cmds[] = {	"append",	2,	"append file1 onto file2",
		...
		"?",		0,	"synonym for 'help'"};
#define	NCMDS	sizeof(cmds)/sizeof(struct cmdname)
int	ncmds;
...
duhelp()
{
	...
	for (i=0 ; i<ncmds ; i++) {
		kprintf("   %s",cmds[i].cname);
		for (len=strlen(cmds[i].cname) ; len<16 ; len++)
			kprintf(" ");
		kprintf("%d    %s\n",cmds[i].cargs, cmds[i].cexpl);
	}
	kprintf("\n");
	return(OK);		
}
...
/*------------------------------------------------------------------------
 *  parse  -- parse one command line and return command index
 *------------------------------------------------------------------------
 */
LOCAL	parse()
{
...
int	len;
{
	...
	for (i=0 ; i<ncmds ; i++)
		if (len <= strlen(cmds[i].cname) &&
		    strncmp(cmds[i].cname, buf, len) == 0)
			return(i);
	return(SYSERR);
}
